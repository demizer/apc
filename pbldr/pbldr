...
def set_new_sums(package_path):  # {{{
    ...
    # makepkg shouldn't be run as root user, but in this case we are only
    # building the package sources.
    cmd = ['makepkg', '-c', '-g', '--asroot']
    logger.info(str(cmd))
    proc = subprocess.Popen(cmd, stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE)
    sout, serr = proc.communicate()
    if proc.returncode:
        logger.warning(serr.decode())

    smsum = re.findall(r'\ *(?:sha|md)\d+sums=\([\w\s\n\']+\)',
                       sout.decode('UTF-8'))
    if len(smsum) > 1:
        logger.warning('md5sums regex error: More than one match!')
        logger.warning(str(smsum))
...
class App:  # {{{
    ...
    def __init__(self):  # {{{
        ...
        # Check for root priviledges early for the build command since we need
        # write access to the chroot
        if self.args.subparser_name == 'build':
            # Check for root priviledges
            if not check_user_root():
                logger.critical('Error: the build command needs root '
                                'priviledges')
                sys.exit(1)
    ...
    def load_configuration(self):  # {{{
        ...
        self.build_order = jobj['PackageBuildOrder']
    ...
    def chroot_clean(self):  # {{{
        ...
        for arch in ('x86_64', 'i686'):
            ...
            proc = subprocess.call(['mkdir', '-p', copydir])
            if proc > 0:
                logger.warning('Error: could not create directory')
            log('Creating clean chroot at ' + copydir)
            proc = subprocess.call(['rsync', '-a', '--delete', '-q', '-W',
                                    '-x', cdir + '/root/', copydir])
            if proc > 0:
    ...
    def chroot_install_local_dependencies(self, package, arch):  # {{{
        ...
        logger.debug('Dependencies: ' + str(deps))
        i_pkg_list = []
        for pkg in deps:
            if '=' in pkg:
                ...
                pkgname = pkg
                pkgvers = ''
            dep = self.get_local_dependency(pkgname, pkgvers, arch)
            if dep:
    ...
    def chroot_install_packages(self, packages, arch):  # {{{
        ...
        for pkg in packages:
            ...
            checksig = True
            for dpkg, dobj in self.packages.items():
                if pkg == dobj['dest'][arch]:
                    checksig = False
            if checksig:
                log('Checking signature for ' + pkg)
                cmd = ['pacman-key', '-v', pkg + '.sig']
                logger.debug(" ".join(cmd))
                proc = subprocess.call(cmd)
                if proc > 0:
            ...
            log('Copying ' + pkg + ' to the chroot')
            cmd = ['cp', pkg, os.path.join(copydir, pbname)]
            logger.debug(" ".join(cmd))
            proc = subprocess.call(cmd)
            if proc > 0:
            ...
            if proc > 0:
                logger.critical('There was a problem installing the package!')
                sys.exit(1)

            cmd = ['rm', os.path.join(copydir, pbname)]
            logger.debug(" ".join(cmd))
            proc = subprocess.call(cmd)
            if proc > 0:
        ...
        for arch in ('x86_64', 'i686'):
            for pkg in self.packages:
                ...
                if not os.path.exists(pkg_full_path):
                    log(os.path.basename(pkg_full_path) + ' does not exist')
                else:
    ...
    def get_local_dependency(self, name, version, arch):  # {{{
        ...
        logger.debug('Get dep regex: ' + repat)
        stage_path = os.path.join(self.base_path, 'stage')
        dep_path = os.path.join(self.base_path, 'depends')
    ...
    def build_package(self, package, arch):  # {{{
        ...
        if overwrite:
            ...
            rmcmd = 'rm -rf ' + os.path.join(cdir, copydir + '/build/*')
            logger.info(rmcmd)
